...
foreach ($logs as $log) {
    if ($log['customer_type'] === 'Walk-in') $walkIns++;
    if ($log['customer_type'] === 'Reservation') $reservations++;
    if (is_null($log['check_out_time'])) $currentlyCheckedIn++;
}
?>
...
<body>
        ...
        <div class="logbook-container">
            <div class="toolbar">
                <form class="search-form" method="GET" action="logbook.php">
                    <input type="text" name="search" class="search-input" placeholder="Search transactions, customers, accommodations..." value="<?= htmlspecialchars($search) ?>">
                ...
                <tbody>
                    <?php if (count($logs) > 0): ?>
                        <?php foreach ($logs as $row): ?>
                            <tr>
                                <td style="font-weight: 600; color: var(--text-dark);"><?= htmlspecialchars($row['customer_name']) ?></td>
                                ...
                                <td style="display:flex; gap: 5px;">
                                    <?php if (!$row['check_out_time']): ?>
                                        <button class="btn btn-success" style="padding: 6px 10px; font-size: 12px;" onclick="openCheckoutModal(<?= $row['id'] ?>)">Out</button>
                                    <?php endif; ?>

                                    <button type="button" class="btn btn-primary" style="padding: 6px 10px; font-size: 12px;" onclick="openEditModal(<?= $row['id'] ?>, '<?= htmlspecialchars(addslashes($row['customer_name'])) ?>', <?= $row['pax'] ?>, '<?= htmlspecialchars(addslashes($row['contact_number'])) ?>', '<?= $row['customer_type'] ?>', '<?= $row['overnight'] ?>', '<?= htmlspecialchars(addslashes($row['accommodation'])) ?>')">Edit</button>

                                    <form method="POST" action="actions.php" style="margin:0;">
                                        <input type="hidden" name="action" value="delete">
                                        <input type="hidden" name="customer_id" value="<?= $row['id'] ?>">
                                        <button type="submit" class="btn btn-danger" style="padding: 6px 10px; font-size: 12px;" onclick="return confirm('Delete this record?');">Del</button>
                                    </form>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr>
                            <td colspan="9" style="text-align:center; padding: 30px; color: var(--text-muted);">No records found.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
        ...
        <div class="modal-content">
            ...
            <form method="POST" action="actions.php">
                ...
                <div class="modal-actions">
                    <button type="button" class="btn btn-clear" style="background:none; color: var(--text-muted);" onclick="closeModal('addModal')">Cancel</button>
    ...
    <div id="editModal" class="modal">
        <div class="modal-content">
            ...
            <form method="POST" action="actions.php">
                ...
                <div class="modal-actions">
                    <button type="button" class="btn btn-clear" style="background:none; color: var(--text-muted);" onclick="closeModal('editModal')">Cancel</button>
    ...
    <div id="checkoutModal" class="modal">
        <div class="modal-content" style="width: 400px;">
            ...
            <p style="color: var(--text-muted); font-size: 14px; margin-bottom: 20px;">The check out time will be recorded as right now. Proceed?</p>
            <form id="checkoutForm" method="POST" action="actions.php">
                <input type="hidden" name="action" value="checkout">
                <input type="hidden" name="customer_id" id="checkout_customer_id">
                <div class="modal-actions">
                    <button type="button" class="btn btn-clear" style="background:none; color: var(--text-muted);" onclick="closeModal('checkoutModal')">Cancel</button>
