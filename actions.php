...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'add' || $action === 'edit') {
        ...
        $acc_fee = $_POST['accommodation_fee'] ?? 0;
        
        // Capture total_amount from the hidden input, fallback to calculation just in case
        $total_amount = $_POST['total_amount'] ?? ($entrance_fee + $acc_fee);
        
        $payment = $_POST['payment_status'];
        $check_in = $_POST['check_in_time'];

        if ($action === 'add') {
            $stmt = $pdo->prepare("INSERT INTO customer_logs (customer_name, pax, customer_type, overnight, accommodation, contact_number, entrance_fee, accommodation_fee, total_amount, payment_status, check_in_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$name, $pax, $type, $overnight, $accommodation, $contact, $entrance_fee, $acc_fee, $total_amount, $payment, $check_in]);
            ...
            $stmt = $pdo->prepare("UPDATE customer_logs SET customer_name=?, pax=?, customer_type=?, overnight=?, accommodation=?, contact_number=?, entrance_fee=?, accommodation_fee=?, total_amount=?, payment_status=?, check_in_time=? WHERE id=?");
            $stmt->execute([$name, $pax, $type, $overnight, $accommodation, $contact, $entrance_fee, $acc_fee, $total_amount, $payment, $check_in, $id]);
        }
    ...
    elseif ($action === 'update_payment') {
        ...
        // Ensure values are treated as floats to prevent database errors
        ...
        $total_amount = $entrance_fee + $accommodation_fee;

        // Update the invoice details in the database
        $stmt = $pdo->prepare("UPDATE customer_logs SET entrance_fee = ?, accommodation_fee = ?, total_amount = ?, payment_status = ? WHERE id = ?");
        $stmt->execute([$entrance_fee, $accommodation_fee, $total_amount, $payment_status, $customer_id]);

        // Send the user back to the payments dashboard
