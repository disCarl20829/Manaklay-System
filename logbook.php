...
<body>
        ...
        <div class="modal-content" style="width: 600px;">
            ...
            <form method="POST" action="actions.php">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group"><label>Adults</label><input type="number" name="adults" id="adults" min="0" oninput="calculateFees()"></div>
                    <div class="form-group"><label>Seniors / PWD</label><input type="number" name="seniors" id="seniors" min="0" oninput="calculateFees()"></div>
                    <div class="form-group"><label>Children</label><input type="number" name="children" id="children" min="0" oninput="calculateFees()"></div>
                    <div class="form-group"><label>Contact</label><input type="text" name="contact_number" required></div>

                    <div class="form-group form-group-full">
                        <label>Accommodation</label>
                        <select name="accommodation" id="accommodation_select" onchange="updateAccommodationPrice()" required>
                            <option value="" data-price="0">-- Select Room/Cottage --</option>
                            <?php foreach ($accommodationsList as $acc): ?>
                                <option value="<?= $acc['type'] . ' ' . $acc['number'] ?>" data-price="<?= htmlspecialchars($acc['price_per_day']) ?>">
                                    <?= $acc['type'] . ' ' . $acc['number'] ?> (₱<?= number_format($acc['price_per_day'], 2) ?>)
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>

                    <div class="form-group"><label>Accommodation Fee (₱)</label><input type="number" step="0.01" name="accommodation_fee" id="accommodation_fee" oninput="calculateFees()"></div>
                    <div class="form-group">
                    ...
                    <div class="form-group form-group-full" style="background: var(--bg-light); padding: 15px; border-radius: 8px; text-align: right;">
                        <span style="font-size: 14px; color: var(--text-muted);">Total Expected Fees:</span>
                        <strong style="font-size: 24px; color: var(--accent-orange); display: block;">₱<span id="display_total">0.00</span></strong>
                    </div>
                </div>
        ...
        <div class="modal-content" style="width: 600px;">
            ...
            <form method="POST" action="actions.php">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group"><label>Adults</label><input type="number" name="adults" id="edit_adults" min="0" oninput="calculateEditFees()"></div>
                    <div class="form-group"><label>Seniors / PWD</label><input type="number" name="seniors" id="edit_seniors" min="0" oninput="calculateEditFees()"></div>
                    <div class="form-group"><label>Children</label><input type="number" name="children" id="edit_children" min="0" oninput="calculateEditFees()"></div>
                    <div class="form-group"><label>Contact</label><input type="text" name="contact_number" id="edit_contact_number" required></div>

                    <div class="form-group form-group-full">
                        <label>Accommodation</label>
                        <select name="accommodation" id="edit_accommodation_select" onchange="updateEditAccommodationPrice()" required>
                            <option value="" data-price="0">-- Select Room/Cottage --</option>
                            <?php foreach ($accommodationsList as $acc): ?>
                                <option value="<?= $acc['type'] . ' ' . $acc['number'] ?>" data-price="<?= htmlspecialchars($acc['price_per_day']) ?>">
                                    <?= $acc['type'] . ' ' . $acc['number'] ?> (₱<?= number_format($acc['price_per_day'], 2) ?>)
                    ...
                    <div class="form-group"><label>Accommodation Fee (₱)</label><input type="number" step="0.01" name="accommodation_fee" id="edit_accommodation_fee" oninput="calculateEditFees()"></div>
                    <div class="form-group">
                        <label>Payment Status</label>
                    ...
                    <div class="form-group form-group-full" style="background: var(--bg-light); padding: 15px; border-radius: 8px; text-align: right;">
                        <span style="font-size: 14px; color: var(--text-muted);">Total Expected Fees:</span>
                        <strong style="font-size: 24px; color: var(--accent-orange); display: block;">₱<span id="display_edit_total">0.00</span></strong>
                    </div>
                </div>
    ...
    <script>
        ...
        function openEditModal(id, name, type, overnight, contact, accommodation, acc_fee, entrance_fee, payment_status, check_in_time, total_amount) {
            // 1. Target the new edit_ IDs
            ...
            document.getElementById('edit_accommodation_select').value = accommodation;
            ...
            document.getElementById('edit_check_in_time').value = check_in_time;

            // Note: Since your database only stores total 'pax' and 'entrance_fee' 
            // (not the exact breakdown of adults/children), we will leave the demographic 
            // inputs blank for them to re-enter IF they want to recalculate, BUT we will 
            // display their existing totals.
            document.getElementById('hidden_edit_entrance_fee').value = entrance_fee;
            document.getElementById('hidden_edit_total_amount').value = total_amount;
            document.getElementById('display_edit_total').innerText = parseFloat(total_amount).toFixed(2);

            // 2. Open the modal
            document.getElementById('editModal').style.display = 'flex'; // or 'block' depending on your CSS
        }
