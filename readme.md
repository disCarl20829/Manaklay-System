
# Manaklay Beach & Park Resort—Multisystem

A multisystem project to be used for a beach resort in handling services such as accounting and inventory, cashier system, customer logbook systems. This to ensure and enhance proper managing.



## Environment Variables and Requirements

To run this project, you will need to add the following environment variables to your .env file

### Variables

`vendor` for using PHP related environment variables

`phpdotenv` For compiling every variable the system Requirements

### Applications

`Cloudflare` to run a simple and free server to be accessible remotely

`Composer` to install dependencies for PHP



## Tech Stack

**Client:** HTML, CSS, Vanilla JavaScript

**Server:** PHP, Python


## Authors

- [@deover](https://github.com/de0-14)
- [@disCarl20829](https://github.com/disCarl20829)
- [@ikatulonapipi](https://github.com/IkaTuloNaP-pipi)
